...
import com.yc.springcloud812.security.domain.Book;
...
public class ConsumerBookController {
    ...
    public Book getBook(@PathVariable("id") Integer id){
        ...
        //   返回: ResponseEntity
        //根据服务名获得(服务实例，包括了服务清单)
        ServiceInstance serviceInstance=this.loadBalancerClient.choose("MICROSERVICE-PROVIDER");
        String ip=serviceInstance.getHost();
        int port=serviceInstance.getPort();
        System.out.println("[服务提供实例信息]host= "+ip+"  port= "+port+"  serviceId= "+serviceInstance.getServiceId());
